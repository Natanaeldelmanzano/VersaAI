#!/usr/bin/env python3
"""
Pruebas de Integración Completas con Setup - VersaAI
Crea un usuario de prueba y verifica todos los endpoints
"""
...
def setup_test_user():
    ...
    try:
        ...
        if response.status_code == 201:
            log_test("Registro de Usuario de Prueba", "PASS", "Usuario creado exitosamente")
            return True
        elif response.status_code == 400 and "already exists" in response.text:
            log_test("Usuario de Prueba", "INFO", "Usuario ya existe, continuando...")
            return True
        else:
            log_test("Registro de Usuario de Prueba", "FAIL", f"Status: {response.status_code}, Error: {response.text}")
            return False
            
    except Exception as e:
        log_test("Registro de Usuario de Prueba", "FAIL", f"Excepción: {str(e)}")
        return False
...
def test_chatbot_endpoints(token):
    ...
    chatbot_data = {
        "name": f"Integration Test Bot {timestamp}",
...
def test_frontend_connectivity():
    ...
    print("🌐 PRUEBAS DE FRONTEND")
    print("=" * 50)
    
    try:
        response = requests.get("http://localhost:3000/", timeout=5)
        if response.status_code == 200:
            log_test("Servidor Frontend", "PASS", "Frontend accesible")
        else:
            log_test("Servidor Frontend", "FAIL", f"Status: {response.status_code}")
    except Exception as e:
        log_test("Servidor Frontend", "FAIL", f"Excepción: {str(e)}")

def main():
    """Ejecuta todas las pruebas de integración"""
    print("🚀 INICIANDO PRUEBAS DE INTEGRACIÓN COMPLETAS CON SETUP - VersaAI")
    print("=" * 80)
    ...
    if not setup_test_user():
        print("❌ No se pudo configurar el usuario de prueba. Deteniendo pruebas.")
        return
    ...
    print("🤖 Chatbots pueden ser creados y gestionados exitosamente")
    print()
